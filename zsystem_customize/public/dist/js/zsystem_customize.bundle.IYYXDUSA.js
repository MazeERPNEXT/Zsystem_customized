(() => {
  ...
  // ../zsystem_customize/zsystem_customize/public/js/material_request.js
  ...
  frappe.ui.form.on("Material Request Item", {
    item_code: function(frm, cdt, cdn) {
      ...
      frappe.db.get_value("Item", row.item_code, "custom_warehouse").then((r) => {
  ...
  // ../zsystem_customize/zsystem_customize/public/js/bom.js
    ...
    refresh: function(frm) {
      ...
      if (frm.doc.docstatus == 0) {
        frm.add_custom_button(
          __("Purchase Order"),
          function() {
            let selected_rows = frm.fields_dict.items.grid.get_selected_children();
      ...
      frm.fields_dict.items.grid.wrapper.on(
        ...
        function() {
          setTimeout(function() {
            let selected_rows = frm.fields_dict.items.grid.get_selected_children();
            let btn = frm.page.wrapper.find(".dropdown-menu li").filter(function() {
              return $(this).text().trim() === __("Purchase Order");
            });
      ...
      frm.fields_dict.items.grid.wrapper.on(
        ...
        function() {
          setTimeout(function() {
            let selected_rows = frm.fields_dict.items.grid.get_selected_children();
            let btn = frm.page.wrapper.find(".dropdown-menu li").filter(function() {
              return $(this).text().trim() === __("Material Request");
            });
            if (selected_rows.length > 0) {
              btn.show();
            } else {
  ...
  function create_purchase_order(frm, selected_rows) {
    ...
    frappe.call({
      method: "zsystem_customize.bom.get_purchase_order_data_from_bom",
      args: {
        bom: frm.doc.name,
        selected_items: JSON.stringify(selected_items)
      },
      freeze: true,
      freeze_message: __("Fetching selected BOM items..."),
      callback: function(r) {
        if (!r.message) {
          return;
        }
        ...
        frappe.model.with_doctype(
          ...
          function() {
            ...
            data.items.forEach(function(item) {
              let po_item = frappe.model.add_child(
                po,
                "Purchase Order Item",
                "items"
              );
  ...
  function create_material_request(frm, selected_rows) {
    ...
    console.log("Selected BOM Items:", selected_items);
    frappe.call({
      method: "zsystem_customize.bom.get_material_request_data_from_bom",
      args: {
        bom: frm.doc.name,
        selected_items: JSON.stringify(selected_items)
      },
      ...
      callback: function(r) {
        ...
        frappe.model.with_doctype(
          ...
          function() {
            ...
            data.items.forEach(function(item) {
              let mr_item = frappe.model.add_child(
                mr,
  ...
  // ../zsystem_customize/zsystem_customize/public/js/work_order.js
  ...
  // ../zsystem_customize/zsystem_customize/public/js/stock_entry.js
