...
def execute(filters=None):
	...
	set_rate_from_voucher(sl_entries)
...
def set_rate_from_voucher(sl_entries):
	...
	for sle in sl_entries:
		...
		if not sle.voucher_type:
			continue

		if sle.voucher_type not in voucher_doctype_map:
			continue

		config = voucher_doctype_map[sle.voucher_type]
		...
		if config.get("parent_doctype") and config.get("party_field"):
			...
			if config["party_field"] == "customer":
				sle["customer"] = party

			elif config["party_field"] == "supplier":
				sle["supplier"] = party
...
def get_items(filters):
	...
	if item_codes := filters.get("item_code"):
		...
		# if item_code is list/tuple
		if isinstance(item_codes, (list, tuple)):
			conditions.append(item.name.isin(item_codes))

		# if single item_code string
		else:
			conditions.append(item.name == item_codes)
...
def get_opening_balance_from_batch(filters, columns, sl_entries):
	...
	for field in ["item_code", "warehouse", "company","voucher_type"]:
